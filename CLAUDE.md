...
**レイアウトの確立パターン（変更時は踏襲する）：**
- fixed header + sticky footer
- ページタイトルはヘッダーに統合（独立した `<h1>` タグを別途置かない）
- ハンバーガーメニューはヘッダー左上に配置
- スライダーは slick（jQuery）を使用

**View を変更したら `designer-reviewer` エージェントでレビューを行う。**
レイアウト・視覚的階層・余白・カラー一貫性・アクセシビリティ・レスポンシブの観点で確認する。
コードを変更するレビューではなく、指摘を受けてから対応を判断する。
